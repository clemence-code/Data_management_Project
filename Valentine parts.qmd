...
This data set shows the HDI rank for all countries in 2023 compared to the 2022 rank. It displays the life expectancy at birth (in years), the expected years of schooling and the Gross National Income per capita (in 2021 PPP $) used to compute the HDI.
...
This dataset displays the share of net personal wealth hold by the Top10 percentile of the population by country and at the world aggregate for the years 1990 to 2024.
```{r}
...
This data set references the electric power consumption in kWh per capita, for all countries with available data from 1960 to 2024.

```{r}
PC_Energy_Use<- vroom(here("data_file", "API_EG.USE.ELEC.KH.PC_DS2_en_csv_v2_129274.csv"), skip=4)
```
```{r}
PC_Energy_Use |> 
  summarise(
