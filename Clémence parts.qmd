...
## 2.6 Countries without iso3c match

```{r unmatched_iso3}
unmatched_iso3_countries <- country_list |>
  imap_dfr(
    ~ .x |>
      filter(is.na(iso3c)) |>
      distinct(Country) |>
...
unmatched_countries_unique <- unmatched_iso3_countries |>
  distinct(Country) |>
  arrange(Country)

unmatched_iso3_countries |>
  knitr::kable(caption = "Countries without iso3c match (by dataset)")
```
