...
```{r}
Temperature_change_summary <- bind_rows(
  Temperature_change |>
    summarise(
      number_of_rows = n(),
...
Temperature_change_summary |>
  knitr::kable(caption = "Summary of Temperature exposure dataset")
```
...
```{r}
Top_10_income_share_summary <- bind_rows(
...
Top_10_income_share_summary |>
  knitr::kable(caption = "Summary of Top 10 income share dataset")

```

### 1.3.4 Expenditure on environmental protection (IMF)

https://climatedata.imf.org/pages/fiscal-and-trade-related-indicators#id1
...
Exp_env_summary |>
  knitr::kable(caption = "Summary of Expenditure on environmnental protection")
```
...
### 1.3.8 CO2 emissions per capita
...
PC_Energy_Use <- PC_Energy_Use |>
  filter(!Country %in% non_country_units)
...
```{r}
all_countries <- c(
  PC_Energy_Use$Country,
  RE_Electricity_Share_clean$Country,
  Top_10_income_share_clean$Country
) |> 
  unique()

unused_overall <- setdiff(non_country_units, all_countries)
used_overall   <- intersect(non_country_units, all_countries)
...
Exp_env_long         <- fix_iso3(Exp_env_long)
...
```{r}
sapply(country_list, \(df) dplyr::n_distinct(df$iso3c))
country_coverage_wide |>
  summarise(across(-iso3c, ~ sum(.)))
countries_not_matched <- country_coverage_wide |>
  filter(!(iso3c %in% matched_iso3))
